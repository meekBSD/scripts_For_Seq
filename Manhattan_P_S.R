...
manhattan <- function(dataframe, limitchromosomes=NULL,pt.col=c('blue','orange'),pt.bg=c('blue','orange'),

    pt.cex=0.45,pch=21,cex.axis=0.95,gridlines=F,gridlines.col='gray83',gridlines.lty=1,gridlines.lwd=1,ymax=10, ymax.soft=T, annotate=NULL,annotate.cex=0.7,annotate.font=3,

    suggestiveline=-log10(1e-7), suggestiveline.col='blue', suggestiveline.lwd=1.5, suggestiveline.lty=1,

    genomewideline=-log10(5e-9), genomewideline.col='red', genomewideline.lwd=1.5, genomewideline.lty=1,
...
if (!is.null(annotate)){
   ...
   # REF: https://www.biostars.org/p/10454/
}
...
ass_files = Sys.glob(paste("./","GWAS_*.qassoc",sep=''))

for (i in ass_files){
    d <- read.table(i, header=T)
    d1 <- subset(d, select=c("CHR", "SNP", "BP", "P"))
    dn3 <-d1[complete.cases(d1[,4]),] 
   # dn4 <-d1[complete.cases(d1),]
    d4 <- subset(dn3, P<0.05 )
    pdf(paste(i,".pdf",sep=""))
    manhattan(d4, pt.col=c(1:20))
    dev.off()
    print(i)
}
